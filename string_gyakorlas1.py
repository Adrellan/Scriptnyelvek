...
def fix_start(s):
    ...
    """
    FELADAT FÉLREÉRTÉS!!!
    Az alábbi sorokkal minden karakterből csak egyszer szerepelhet
    ...
    arr=list(s)
    for c in s:
        #print("Én vagyok c: "+c+"   ezen a helyen: {i}".format(i=kulso_index))
        kulso_index+=1
        belso_index=kulso_index
        for c2 in s[kulso_index:l:1]:
            #print("Én vagyok c2: "+c2+"   ezen a helyen: {i}".format(i=belso_index))
            if(c==c2):
                arr[belso_index]='*'
            belso_index+=1
    ...
    l=len(s)
    arr=list(s)
    for index, value in enumerate(arr[1:l:1]):
        if(value==s[0]):
            #print(index, value)
            arr[index+1]='*'
